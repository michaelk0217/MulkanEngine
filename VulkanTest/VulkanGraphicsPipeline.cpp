...
void VulkanGraphicsPipeline::createSkybox(VkDevice vkDevice, VkPipelineLayout pipelineLayout, VkRenderPass renderPass, const std::string& vertShaderPath, const std::string& fragShaderPath)
{
	...
	// The attribute description tells the shader how to interpret the data within a single vertex.
	// We are creating one specifically for the skybox shader.
	VkVertexInputAttributeDescription attributeDescription{};
	attributeDescription.binding = 0; // We're using the first and only binding.
	attributeDescription.location = 0; // <-- This MUST match `layout(location = 0)` in the shader.
	attributeDescription.format = VK_FORMAT_R32G32B32_SFLOAT; // The format of a vec3 position.
	attributeDescription.offset = 0; // Position data is at the beginning of the Vertex struct.
	...
	rasterizer.lineWidth = 1.0f;
	rasterizer.cullMode = VK_CULL_MODE_FRONT_BIT; // Cull front faces because we are inside the cube (skybox)
...
void VulkanGraphicsPipeline::createForConversion(VkDevice device, VkPipelineLayout pipelineLayout, VkRenderPass renderPass, const std::string& vertShaderPath, const std::string& fragShaderPath)
{
	this->device = device;
	...
	rasterizer.polygonMode = VK_POLYGON_MODE_FILL;
	rasterizer.lineWidth = 1.0f;
	rasterizer.cullMode = VK_CULL_MODE_NONE; // NONE to be safe, or BACK_BIT
	rasterizer.frontFace = VK_FRONT_FACE_COUNTER_CLOCKWISE;
	rasterizer.depthBiasEnable = VK_FALSE;
...
void VulkanGraphicsPipeline::createForLutGeneration(VkDevice device, VkPipelineLayout pipelineLayout, VkRenderPass renderPass, const std::string& vertShaderPath, const std::string& fragShaderPath)
{
	this->device = device;
	...
	// --- NO VERTEX INPUT since we are generating vertices
	// We are generating vertices in the shader, so we don't need any vertex buffers.
	VkPipelineVertexInputStateCreateInfo vertexInputInfo{};
	vertexInputInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
	...
	rasterizer.polygonMode = VK_POLYGON_MODE_FILL;
	rasterizer.lineWidth = 1.0f;
	rasterizer.cullMode = VK_CULL_MODE_NONE; // No need to cull the single triangle
	rasterizer.frontFace = VK_FRONT_FACE_COUNTER_CLOCKWISE;
	rasterizer.depthBiasEnable = VK_FALSE;
	...
	// NO DEPTH TESTING since we r rendering a 2D quad
	VkPipelineDepthStencilStateCreateInfo depthStencil{};
	depthStencil.sType = VK_STRUCTURE_TYPE_PIPELINE_DEPTH_STENCIL_STATE_CREATE_INFO;
	depthStencil.depthTestEnable = VK_FALSE;
	depthStencil.depthWriteEnable = VK_FALSE;
	...
	if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS)
	{
		throw std::runtime_error("failed to create LUT generation graphics pipeline!");
	}
